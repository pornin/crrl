...
use crrl::jq255e::{Point, Scalar, PrivateKey};
...
fn main() {
    ...
    let (v, x) = bench_mul();
    bx ^= x;
    println!("Jq255e point mul:              {:13.2}", v);
    let (v, x) = bench_mulgen();
    bx ^= x;
    println!("Jq255e point mulgen:           {:13.2}", v);
    let (v, x) = bench_mul_add_mulgen();
    bx ^= x;
    println!("Jq255e point mul_add_mulgen:   {:13.2}", v);
    let (v, x) = bench_skey_load();
    bx ^= x;
    println!("Jq255e skey_load:              {:13.2}", v);
    let (v, x) = bench_skey_sign();
    bx ^= x;
    println!("Jq255e sign:                   {:13.2}", v);
    let (v, x) = bench_pkey_verify();
    bx ^= x;
    println!("Jq255e verify:                 {:13.2}", v);
    let (v, x) = bench_decode();
    bx ^= x;
    println!("Jq255e decode:                 {:13.2}", v);
    let (v, x) = bench_encode();
    bx ^= x;
    println!("Jq255e encode:                 {:13.2}", v);
